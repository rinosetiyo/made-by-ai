...
const HeroArticle = ({ article }) => (
    <div className="grid md:grid-cols-2 gap-8 items-center mb-12">
        ...
        <div>
            ...
            <Link to={`/articles/${article.id}`}><h1 className="text-4xl font-bold text-gray-900 leading-tight hover:text-red-600">{article.title}</h1></Link>
            <p className="text-gray-600 mt-4">{article.content.substring(0, 150)}...</p>
...
const ArticleCard = ({ article }) => (
    <Link to={`/articles/${article.id}`}>
        <div className="bg-white h-full hover:shadow-xl transition-shadow duration-300 rounded-lg overflow-hidden">
            ...
            <div className="p-4">
                <div className="text-xs text-gray-500 flex items-center"><span>{article.author_detail?.name || article.author || 'News'}</span><span className="mx-2">•</span><span>{new Date(article.publication_date).toLocaleDateString()}</span></div>
                <h3 className="text-lg font-bold text-gray-800 mt-1">{article.title}</h3>
                <div className="text-xs text-gray-500 mt-2"><span>{article.category?.name || 'General'}</span><span className="mx-2">•</span><span>{article.read_time} min read</span></div>
            </div>
        </div>
    </Link>
);
